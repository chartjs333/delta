"""Stable domain errors shared by CLI and artifact boundaries."""

from __future__ import annotations

from collections.abc import Mapping
from enum import StrEnum
from types import MappingProxyType
from typing import Any


class ErrorCode(StrEnum):
    ARTIFACT_HASH_MISMATCH = "ARTIFACT_HASH_MISMATCH"
    ARTIFACT_IMMUTABLE_CONFLICT = "ARTIFACT_IMMUTABLE_CONFLICT"
    ARTIFACT_NOT_FOUND = "ARTIFACT_NOT_FOUND"
    FORMAL_SEMANTICS_MISMATCH = "FORMAL_SEMANTICS_MISMATCH"
    INVALID_CONTENT_ID = "INVALID_CONTENT_ID"
    INVALID_MANIFEST = "INVALID_MANIFEST"
    INVALID_CONTRIBUTION_CANDIDATE = "INVALID_CONTRIBUTION_CANDIDATE"
    INVALID_DELTA_TENSOR = "INVALID_DELTA_TENSOR"
    INVALID_LOCAL_COMPLETION = "INVALID_LOCAL_COMPLETION"
    INVALID_NETWORK_PROFILE = "INVALID_NETWORK_PROFILE"
    INVALID_PARAMETER_SCHEMA = "INVALID_PARAMETER_SCHEMA"
    INVALID_SCHEMA_ID = "INVALID_SCHEMA_ID"
    INVALID_ARTIFACT_LOCATOR = "INVALID_ARTIFACT_LOCATOR"
    INVALID_TOKEN_ACCOUNTING = "INVALID_TOKEN_ACCOUNTING"
    INVALID_WORK_TICKET = "INVALID_WORK_TICKET"
    INVALID_WORKER_TRANSITION = "INVALID_WORKER_TRANSITION"
    SCHEMA_INVALID = "SCHEMA_INVALID"
    NETEM_PLATFORM_UNSUPPORTED = "NETEM_PLATFORM_UNSUPPORTED"
    NETEM_PRIVILEGE_REQUIRED = "NETEM_PRIVILEGE_REQUIRED"
    NETEM_TC_UNAVAILABLE = "NETEM_TC_UNAVAILABLE"
    UNKNOWN_MEDIA_TYPE = "UNKNOWN_MEDIA_TYPE"
    UNSAFE_SERIALIZATION = "UNSAFE_SERIALIZATION"
    TICKET_ALREADY_IN_PROGRESS = "TICKET_ALREADY_IN_PROGRESS"
    TICKET_ID_CONFLICT = "TICKET_ID_CONFLICT"
    WORKER_CANCELLED = "WORKER_CANCELLED"
    WORKER_CRASH = "WORKER_CRASH"
    WORKER_DATA_EXHAUSTED = "WORKER_DATA_EXHAUSTED"
    WORKER_DEADLINE_EXCEEDED = "WORKER_DEADLINE_EXCEEDED"
    WORKER_OOM = "WORKER_OOM"


class DeltaError(RuntimeError):
    """Error with a stable code and non-secret structured details."""

    def __init__(
        self,
        code: ErrorCode,
        message: str,
        details: Mapping[str, Any] | None = None,
    ) -> None:
        super().__init__(message)
        self.code = code
        self.message = message
        self.details = MappingProxyType(dict(details or {}))

    def to_dict(self) -> dict[str, Any]:
        return {
            "code": self.code.value,
            "details": dict(self.details),
            "message": self.message,
        }
