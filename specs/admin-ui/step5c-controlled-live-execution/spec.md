...
5. The terminal receipt binds `intent_id`, `intent_digest`, `admission_id`, `admission_digest`, `execution_id`, catalog ref, producer commit, plugin ID, dataset ID, and executed scope.
...
**Acceptance**:
1. A duplicate active intent (`same intent_id + same intent_digest + same authenticated_subject`) returns the existing `execution_id` and current status without starting another worker.
2. A duplicate completed intent returns the existing terminal result/receipt from cache.
3. Same `intent_id` with a different digest produces an immediate `ERR_INTENT_ID_DIGEST_CONFLICT` rejection.
...
An operator requests `MATERIALIZE_DATASET` (status-only) or `EVALUATE_CHECKPOINT` using only the operation-specific payload allowed by the frozen contract.

**Acceptance**:
1. Each operation has a closed payload schema and an explicit scope allowlist (`TRAIN_TICKET` restricted to `PLUGIN_BOUNDARY`).
...
3. `MATERIALIZE_DATASET` produces typed `ExecutionStatus` only; no `ExecutionReceipt` is emitted in Step 5C v1.
4. `EVALUATE_SPLIT` is excluded from Step 5C v1 to prevent passing in-memory Python objects across boundaries.
5. Operation dispatch is by enum, never by user-supplied function/module/path name.
...
2. Live-mode code uses an explicit, separate `LiveExecutionPort` and does not introduce network egress into offline adapters.
3. Existing CSP `connect-src 'none'` and offline boundaries remain intact during Phase 1-5 UI development.
4. Removing or disabling the live adapter leaves existing browser-local workflows fully operational.
...
- **SR-S5C-002**: Intents MUST expire; the initial maximum TTL is 900 seconds unless a reviewed contract change narrows it further. `admission_expires_at` MUST NOT exceed `intent.expires_at`.
...
- **SR-S5C-005**: The implementation MUST fail closed if canonicalization, digest verification, Ed25519 signature verification, catalog parity, policy evaluation, idempotency state, or lineage verification is unavailable or ambiguous.
...
- **AdmissionRecord**: trusted gate decision signed with Ed25519 binding authenticated authority and resource grants to an exact intent.
...
- **LiveExecutionPort**: isolated Admin UI abstraction for live interaction, maintaining separation from offline `DataSourcePort`.
