...
**Input**: `spec.md`, `plan.md`, Constitution 2.0.0 and completed feature `009`.
...
- [ ] T002 Define `BenchmarkDefinition`, arm, threshold, fault/network profile and compatibility models in `src/deltatorrent/benchmark/definition.py`.
...
- [ ] T006 Create golden definition/run/evidence/result fixtures in `tests/fixtures/contracts/deltareduce_v1/010/`.
- [ ] T007 Add contract/mutation tests in `tests/contract/test_benchmark_definition_bytes.py`.

## Phase 2: Preregistration and completeness

- [ ] T008 Implement definition completeness/license/immutable-dependency validation.
- [ ] T009 Implement benchmark review validator/QC workflow in `src/deltatorrent/benchmark/review.py`.
...
- [ ] T014 Implement isolated benchmark namespace and deterministic run orchestrator in `src/deltatorrent/benchmark/orchestrator.py`.
...
- [ ] T018 Implement deterministic unprivileged network simulator profiles in `src/deltatorrent/benchmark/network_profiles.py`.
...
- [ ] T023 Implement append-only/content-addressed run evidence collector in `src/deltatorrent/benchmark/evidence.py`.
...
- [ ] T026 Implement offline evidence graph verifier in `src/deltatorrent/benchmark/verifier.py`.
...
- [ ] T034 Implement deterministic safety gate analyzer in `src/deltatorrent/benchmark/safety.py`.
...
- [ ] T037 Implement quality joins/statistics/thresholds/missing-run policy in `src/deltatorrent/benchmark/quality.py`.
...
- [ ] T046 Implement deterministic all-mandatory decision function in `src/deltatorrent/benchmark/decision.py`.
...
- [ ] T049 Add GO-all-pass, one-gate-fail, missing-evidence and incompatible-definition tests in `tests/integration/test_go_no_go_decision.py`.
...
- [ ] T053 Run cross-artifact analysis and architecture test `tests/architecture/test_benchmark_cannot_weaken_protocol.py`.
