...
export function SourcedResultsPanel({
  query,
}: {
  readonly query: ResultQuery;
}) {
  if (
  ...
  if (query.state === "UNSUPPORTED") {
    return (
      <div className="capability-state state-unsupported" role="status">
        {t("Unsupported: the advertised source operation is not available.")}
      </div>
    );
  }
    ...
    return (
      <div className="capability-state state-error" role="alert">
        {query.state === "INVALID"
          ...
          : t("Result rejected: the source response is malformed.")}
      </div>
    );
  }
    ...
    <div>
      {query.state === "STALE" ? (
        <div className="capability-state state-stale" role="status">
          {t("Stale sourced data — provenance is shown below.")}
        </div>
      ) : null}
