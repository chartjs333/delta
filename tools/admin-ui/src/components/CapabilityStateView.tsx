...
const stateMessage: Readonly<Record<CapabilityState, string>> = {
  ...
  UNAVAILABLE: "Unavailable: the active source does not provide this capability.",
  LOADING: "Loading source capability information.",
  DEGRADED: "Degraded: the source cannot currently provide this capability.",
  STALE: "Stale: the source advertises this capability, but its data may be old.",
...
export function CapabilityStateView({
  ...
  return (
    <div className={`capability-state state-${state.toLowerCase()}`} role="status">
      <strong><InertText value={capability} /></strong>
      <span>{stateMessage[state]}</span>
    </div>
  );
}

export function SourceSummary({ source }: { readonly source: SourceDescriptor }) {
  return (
    <section aria-labelledby="source-heading">
      <p className="eyebrow">Active source</p>
      <h2 id="source-heading"><InertText value={source.label} /></h2>
      <dl className="source-summary">
        <div><dt>Adapter</dt><dd><InertText value={source.adapterId} /></dd></div>
        <div><dt>Authority</dt><dd><InertText value={source.authorityClass} /></dd></div>
        <div><dt>State</dt><dd><InertText value={source.connectionState} /></dd></div>
      </dl>
      <div aria-label="Source capabilities" className="capability-list">
