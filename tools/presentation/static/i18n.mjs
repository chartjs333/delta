...
const strings = {
  ...
  verificationTitle: ['Verification lab', 'Проверка вычислений'],
  verificationSuccess: ['✓ Reference arithmetic and mutation checks completed', '✓ Эталонные вычисления и проверки подмены завершены'],
  eventOracle: ['Checking the pinned formal Python reference source.', 'Проверка закреплённых исходников формального Python-эталона.'],
  eventVerify: ['Starting a separate process: PARAMETER/APPLY calculations and tampering checks.', 'Запуск отдельного процесса: расчёты PARAMETER/APPLY и проверки подмены.'],
  eventVerified: ['Results checked; report and SHA-256 saved. Native WAL was not run.', 'Результаты проверены; отчёт и SHA-256 сохранены. Native WAL не запускался.'],
