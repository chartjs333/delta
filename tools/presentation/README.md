...
**Слайды RU / EN (26 сентября):** [публикация и восстановление](SLIDES.md).
Оба языка доступны из меню рабочего демо. Актуальные внешние ссылки печатает
`D:/delta-presentation/START-SLIDES.ps1`; код входа прежний.

Сохранённая версия для показа 25 сентября: [Git checkpoint / Восстановление](CHECKPOINT-2026-09-24.md).

Для студента и профессора: [практическое руководство на русском](STUDENT-GUIDE-RU.md)
с браузерными URL, сценариями запусков, объяснениями результатов и шаблоном отчёта.
Адрес Quick Tunnel в руководстве актуален на 25 сентября; после смены туннеля
замените его на адрес из `START-REMOTE.ps1`.
...
Новая демонстрация 25 сентября: **Проверка вычислений / Verification lab**
по `/verification/?lang=ru` или `en`. Она запускает закреплённый Python-эталон
PARAMETER/APPLY, показывает повторяемость байтов и пять реальных отказов на
подменённых входах. Отчёты сохраняются и скачиваются; это `FORMAL_REFERENCE_DEMO`,
не квалификация native runtime. Сценарий показа и границы: [VERIFICATION-LAB.md](VERIFICATION-LAB.md).
